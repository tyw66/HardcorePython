...
def draw_to_svg(points:list[Point], circle:Circle, config):
    pt_radius = circle.radius / 30
    svg_input = input_to_svg(points, pt_radius, config)
    svg_output = output_to_svg(circle, config)
...
def input_to_svg(points:list[Point], point_radius, config):
    style = style_from_config(config['input'])
    label_style = label_style_from_config(config['input'])
    [a,b,c] = points
...
def output_to_svg(circle:Circle, config):
    style = style_from_config(config['output'])
    label_style = label_style_from_config(config['output'])
    return [
